...
class DataParser(object):
    ...
    def parse(self, infile=None, df=None, has_label=False):  # 特征预处理，重要!!!
        ...
        for col in dfi.columns:  # col是dfi的某一列的列名, 即某一个field
            ...
            if col in self.feat_dict.numeric_cols:
                ...
                dfi[col] = dfi[col].map(self.feat_dict.feat_dict[col])  # 把当前列(分类特征)值替换成对应的特征id（不同的分类对应的id不同）
                dfv[col] = 1.  # 将该field所有分类特征值赋值为1

        # list of list of feature indices of each sample in the dataset
        Xi = dfi.values.tolist()  # dfi中保存的是特征的id，dfi本质是一个map，每个value是一个list，所有value组成一个array，dfi.values是去除每列列名，即只保留array，而tolist是将array转List
        # list of list of feature values of each sample in the dataset
        Xv = dfv.values.tolist()  # dfv中保存的是特征的值
        if has_label:
            return Xi, Xv, y  # Xi的每一行是一个样本, 每一列是一个field，Xi[0]描述的是取一个样本
        else:
            return Xi, Xv, ids
